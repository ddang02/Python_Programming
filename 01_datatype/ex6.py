...
# # print(a)                            # str은 원본 수정 불가

# # b = [10, 20, 30]
# # b[0] = 40
# # print(b)  # list는 원본 수정 가능

# # # # 문자열 메소드로 변경해도 원본은 그대로, 새로운 문자열 객체가 생성됨
# # b = a.upper()

# # print(a, b)
# # print(id(a), id(b))  # id() : 객체의 고유 주소값 반환
# # print(a is b)  # A is B : 객체 비교 (주소값 비교)

# # print(id(a[0]), id(a[0]))  # 새로운 문자열 객체를 만들지 않음

# # # # 문자열 연결 시 + 연산도 새로운 문자열 객체를 만듦
# # a = "Hello"
# # print(id(a))
# # a += " Python"
# # print(id(a))  # id 값이 바뀌었음 -> 새로운 문자열 객체가 만들어짐
...
# # start = time.time()  # time.time() : 현재 시간을 초 단위로 반환
# # for i in range(10):
...
# # print(s)
# # print(f"실행 시간: {end - start:.6f}초")    # 숫자가 커지면 list가 더 빠름
...
# print(a[5:5:-1])                    # 출력 안됨 (step이 음수이면 start가 end보다 커야 함)
...
s = "1405"
print(f"{s[0]}학년 {s[1]}반 {int(s[2] * 10 + s[3])}번")  # ✅ "1학년 4반 5번" 출력

# # 2️⃣ 학번을 슬라이싱만을 이용해서 학년, 반, 번호로 잘라내기
print(f"{s[0:1]}학년 {s[1:2]}반 {int(s[2:3]*10 + s[3:4])}번")  # ✅ "1학년 4반 5번" 출력
